...
public class BossHealth : MonoBehaviour
{
    ...
    [Header("Cấu hình Tên Hoạt Ảnh Gục Chết")]
    // Điền tên State hoạt ảnh gục ngã của Boss (ví dụ: "B Animation") trên Inspector
    public string deathAnimationStateName = "B Animation";
    ...
    void Awake()
    {
        ...
        // Lấy Animator đính kèm trên Boss
    ...
    public void TakeDamage(int damage, string dealerTag)
    {
        ...
        if (currentHealth <= 0)
        {
            ...
            // Tắt thanh máu UI của Boss đi ngay khi bại trận
            if (healthUIRoot != null) healthUIRoot.gameObject.SetActive(false);

            // GỌI THẲNG SCRIPT QUYẾT ĐỊNH KẾT LIỄU/THA MẠNG ĐÃ CHUYỂN QUA GỌI TÊN ANIMATION
            BossDecisionTrigger decisionTrigger = GetComponent<BossDecisionTrigger>();
            if (decisionTrigger != null)
            {
                decisionTrigger.ActivateDecisionSequence();
            }
            else
            {
                // Dự phòng: Nếu Boss này không có cơ chế phán quyết, chạy thẳng hoạt ảnh chết chỉ định
                if (_animator != null && !string.IsNullOrEmpty(deathAnimationStateName))
                {
                    _animator.Play(deathAnimationStateName);
                }
    ...
    private void OnTriggerExit2D(Collider2D other)
    {
        ...
        {
            if (healthUIRoot != null)
                removeHealthUI(); // Gọi hàm tắt UI an toàn hoặc đổi về enabled = false tùy bạn
        }
    }

    // Đoạn sửa trigger exit lỗi cú pháp lặp ban đầu của bạn được đưa về dạng an toàn
